...
| `Source/learn_from_correction.py` | sections JSON | ALS XML track names + automation keys | **STALE as of 2026-09-02 (Fable second-lens review of the burn round)** — this row described the code BEFORE Codex's in-flight relative-geometry fix (branch `burn/learner-geometry`), which now unescapes in `_normalise`/`TrackInfo.name`. Re-verify against that branch once it merges; do not trust this row until then. | unknown — depends on the merged fix | none |
...
| `Source/render_check.py` | ALS XML (ElementTree) | arrangement report (loops/tracks) + WAV filenames | **UPDATED 2026-09-02 (Fable second-lens review)** — this row was wrong: render_check.py's automation-aware fixes (32efa36, 44a69db, 1050b4b) added real name-based crossings. `ElementTree` auto-decodes XML entity attributes on parse (`EffectiveName`'s `Value` comes back with a real `&`, verified), so ALS-side track names need no unescaping. The report JSON's `t["name"]` values ARE literally escaped (written by the project's gzip+line-level text-patching convention, not re-parsed), so building `track_bpms` via `html.unescape(t["name"])` in `run_check` IS a real, necessary crossing; the matching `html.unescape(c["track"])` call in `reclassify_source_faithful_silence` is then a harmless no-op on the already-unescaped ALS side, not a second real crossing. `audio_dir / (name + ".wav")` crosses to the WAV filename the same way. | yes (`html.unescape` at both sites named above) | `Tests/test_render_check.py` (multiple `&`-in-name fixtures) |
