...
**O5 — Keep the bass stem beside the drums sidecar. DECIDED AND BUILT (Sam, 2026-08-27: "keep the bass on disk but maybe as an mp3", then "Opus encoded bytes inside the npz sounds like the win").** Shipped same day: `__bassstem.npz` with an **Opus payload inside the npz** — codec-small without a playable file on disk, so the no-mixable-stem-audio line stays honest. Measured on a real master: **2.96 MB vs 68.3 MB float32 (23×)**, correlation 0.9986, exact length restored, 0.5 s load. The saver *decodes what it just wrote* and measures any codec delay per file (a pure-sine probe "measured" a phantom 83 ms that was correlation aliasing — the per-file measurement is what caught it), refusing to Opus below 0.98 correlation and falling back to int16 (also the path on machines whose libsndfile lacks OPUS). Written opportunistically whenever a separation runs anyway; warm drums caches never invalidated; legacy pre-opus sidecars still load. First consumer wired: `refit_grid_from_stem` reads both sidecars cache-first and BACKFILLS them when it does separate (~21 s saved per repair; the backfill was a Codex round-1 blocker — the first version threw the freshly computed bass away).

**O6 — MIK cue points: CLOSED (Sam, 2026-08-27: "they are MIK auto Cue Areas").** Machine-generated, so they do NOT get hint-tier trust. Stay unwired; at most a future low-weight corroboration signal if a starved track needs one. Do not revisit without new provenance.
