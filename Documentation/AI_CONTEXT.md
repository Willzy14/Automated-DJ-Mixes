...
**Four-phase mix pipeline now operational. Beat grids from .asd ticks (gate v3). Stem-kick beat detector built from scratch (2026-06-24) — 1.2ms median vs RB across 35 tracks. Pipeline-readiness work in progress.**

**As of 2026-06-24 (BEAT-GRID BAKE-OFF + STEM-KICK DETECTOR — pipeline-readiness sprint):**
- **Beat-grid bake-off SETTLED (35 tracks).** RB ≈ Beat This! tied (12.4 vs 11.8ms median). Constant-grid refinement dead end (confirmed Sam's hard rule). Beat-This! is a fragility/shippability swap, not accuracy upgrade — deferred.
- **Stem-kick detector built: `Source/automated_dj_mixes/stem_grid.py`.** Sam + Claude designed algorithm: kick onsets (sub <150Hz) → snare onsets (200–3000Hz) → period histogram → smooth spline grid (NOT raw onsets — warp jitter fix) → downbeat fusion (snare backbeat + drop-entry kicks). Sam's "sub locates, click times" insight: refine_to_click() snaps from mushy sub-band to sharp >1.5kHz beater transient.
- **35-track corpus result: median 1.2ms vs RB; 32/35 within 10ms, 34/35 within 20ms.** 26/35 clean. 9 self-flagged correctly: 3 LOWC (non-4-to-floor), 5 DB? (downbeat uncertain, grid fine), 1 FAR (Adam Ten 36ms, low spine 172/393 kicks).
- **Key fixes discovered:** warp jitter (raw onsets → UnivariateSpline); late sub onset (refine_to_click); greedy chain desync on fills (→ global beat-index + lstsq).
- **Critical warp lesson (Sam's Ableton test):** numerical grid-vs-kick = 0.0ms can still FAIL in Ableton if markers aren't smooth. Only an Ableton warp-test catches this.
- **Pipeline swap seam:** `orchestrator.py:504` (`calculate_warp_markers_from_beat_grid`). Detector output needs: grid_times array + downbeat phase + extrapolation to full file.
- **Open for pipeline-readiness:** Adam Ten FAR flag, bass-stem downbeat cue (DB? tracks), snare-primary fallback test (LOWC), snare-coincidence mix gate, full-file extrapolation, wire into orchestrator.
