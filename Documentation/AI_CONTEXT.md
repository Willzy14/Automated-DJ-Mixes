...
- Important dropouts may need protected silence followed by a deliberate outgoing accent return; raw kick gaps inside already kickless material should remain evidence rather than structural clips.
...
> **TOP (2026-07-16) - replay the strongest Sam-Tweaks lessons without overfitting.** The source-aware diff and report are complete. Next implementation slice: model independent transition entry/protected-window/accent-return/swap/exit anchors; add evidence-backed extended transitions and incoming intro loops; separate raw dropout overlays from structural promotion; and score loop sources across the full late-track region. Rebuild a fresh or held-out mix and compare blind before changing production defaults. T4's gap/ride is confirmed deliberate. Exclude T3's incidental 12-beat loop from learning in favour of 4/8-beat short-loop candidates; keep only the non-integer tail edits unresolved.
