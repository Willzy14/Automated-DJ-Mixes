...
> **TOP (2026-09-15): burn list substantially worked since creation — A5, B3, C5, C2 done; B1
> resolved (2/4 flags on, 2/4 correctly left off with real reasons). NEXT SESSION, Sam's own
> explicit direction: build the `LOOP_SELF_SIMILARITY_TIERA` "AND semantics" rebuild FIRST**,
> before anything else — `align_engine.py`'s `_loop_self_similarity`/`evaluate_loop_quality`
> currently blend into one replacement score; split into two separate scores (base 5-key +
> tiera-augmented) and fail the loop-quality gate if EITHER fails, then re-run the real
> 15,268-window historical corpus replay to confirm it only ADDS the 791 evidenced catches
> without reintroducing any of the 404 real un-catches, before flipping the default on. See the
> 2026-09-15 Current State entry above for full detail on what was resolved and why. After that:
> revise `Documentation/Plans/swap-first-redesign/c6-c7-c8-plan.md` (sitting in
> `.claude/worktrees/codex-c6c7c8-review/`) against Codex's 8 round-1 findings and get a round-2
> review before building C6. `BASS_RESIDUAL_ENABLED` stays parked (weak real-world case even
> fixed — 4/79 real masters qualify) unless Sam names a specific mix where the bass cutoff
