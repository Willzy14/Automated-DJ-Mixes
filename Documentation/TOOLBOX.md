...
Key functions: `propose_arrangement()` (accepts `hints_path`, `mix_plan_path` and `decisions_path`), `validate_arrangement_plan()` (hard final-geometry gate), `analyse_overlap()` (loop planning + recomputation), `find_similar_pairs()`, `generate_report()`.

**Claude-arranged mode (added 2026-09-15 by Claude, burn list D13).** `--decisions FILE` reads a JSON list of per-transition decisions (`pair_index`, `entry_out_bar`, `intro_trim_bars`, `swap_in_bar`, optional `tail_loop` / `outgoing_cut` / `outro_skip`, plus `swap_cue`/`out_cue`/`reason` audit text). Pairs it names are built by `align_engine.alignment_from_decision` / `fills_from_decision` (policy `claude_decisions_v1`) instead of the anchor search; unnamed pairs and everything after alignment are the production path. `_plan_marker_loops` gained the `outgoing_cut` and `outro_skip` kinds and lets a decision loop a track with no outro (copies after its last clip). The MixPlan records `arrangement_decisions: <file>:<sha256[:12]>` in `human_overrides`. First use: `Test Project/15.09.26 August Releases Mix/Hints/arrangement_decisions.json`.
...
Key functions: `validate_loop_spec()`, `apply_loops()` (preflighted batch), `clone_clip()`, `decompress_als()` / `compress_als()`, `find_track_line_ranges()`, `shift_track_clips()`, `trim_named_clip_front()` (incoming intro trims: later start, gap left for the outgoing), `split_clip_skip_before_end()` (middle skip keeping the ending), `cut_named_clip_front_and_pull()` (added 2026-09-15: removes the front of a named OUTGOING clip and pulls it and every later clip in, no gap - a decision's front cut).

### `Source/apply_automation.py`
**Added 2026-05-21; contract fix 2026-07-16; content-aware margin 2026-09-13.** Volume crossfades (Utility Gain) + EQ bass kills (ChannelEQ LowShelfGain) applied to an arranged Sections .als. Three transition styles auto-selected by overlap length ONLY (unchanged 2026-09-13, see burn list item C2 for the known gap this leaves): **STANDARD** (24-36 bars, existing two-phase model), **LONG_BLEND** (>36 bars, linear crossfade, partial EQ, delayed bass swap by 32 beats), **QUICK_SWAP** (<24 bars, instant swap, no sneak, full EQ kill). Explicit arrangement-report swaps are preserved at valid overlap-start/loop boundaries; the overlap-end fade-room guard now checks `outgoing_has_post_swap_content` (from `align_engine.py`, landmark-policy transitions only — legacy/non-landmark policies keep the original overlap-length-only 4-vs-8-beat margin unconditionally) instead of overlap length alone, so a long overlap whose outgoing genuinely has nothing left to fade gets the same lenient margin a short overlap always got. Bass shelf breakpoints land on the exact same beat as the swap (a hard snap, all styles + two-stage-bass) rather than a beat apart (was a 1-beat ramp until 2026-09-12) — the point-list dedup step now requires both time AND value proximity before merging, so this doesn't collapse back into a ramp. This keeps automation identical to the frozen report and MixPlan reconciliation.
...
Key functions: `find_bass_swap()` (priority-ordered swap point selection), `plan_transitions()` (style selection + rule application + the content-aware margin check), `build_track_automation()` (style-specific envelope point generation, hard bass-shelf snap), `insert_envelopes()` (ALS patching), `_find_audio_dir()` (added 2026-09-12 — walks upward through ancestors to find the project's `Audio/` folder for per-track LUFS leveling; replaced a fixed-depth check that silently missed `build_ab_comparison.py`'s deeper output layout).
...
**2026-09-22 update (Claude, burn list D5 follow-up):** `--write-hints` run standalone (outside the orchestrator) always printed `[skip] no stats` and wrote an empty hints file, even right after a full analysis pass — `detect()`'s only bpm/downbeat fallback for a standalone call read a `Sections Review/Blind_V*` folder from the amplitude blind-viz pipeline retired 2026-06-10, which nothing writes any more. Fixed via new `_resolve_bpm_downbeat_stats()`, which reads bpm from the track's own already-written `_Stem Analysis/SECTIONS_STEM_*.json` first (downbeat always 0.0 by convention), falling back to the retired Blind_V mechanism only for an old project that genuinely still has one. A malformed cache read now prints a `[warn]` instead of silently falling through indistinguishably from "no cache yet" (MiniMax review finding, adopted). `Tests/test_stem_detector_bpm_fallback.py` (6 tests).

**2026-08-25 update (Claude):** `detect(width_cues=True)` (default ON since 2026-09-14) — stereo-width sustained-step boundary candidates feeding the same raw_bounds path as `_energy_cues`; phrase-grid + RMS-flat (≤1.5 dB) + min-4-bar-run gates. Corpus-validated: Revoloution bar 148 + 11 evidenced new boundaries, 0 spurious. Companion flag in `align_engine`: `LOOP_SELF_SIMILARITY_TIERA` (default ON since 2026-09-15) — tiera-feature similarity term for the loop gate. `evaluate_loop_quality` computes TWO separate terms (base 5-key, always; tiera-augmented 10-key, when the flag is on) and fails the check if EITHER is below threshold (AND semantics, not a replacement) — see `Tools/tiera_loop_replay.py` for the corpus-replay methodology and `Documentation/BURN_LIST.md` item B1 for the full evidence trail (80,815 real windows, 831 AND-gate flips, all pass→reject, 0 reject→pass, 816 evidence-backed).
...
**Added 2026-07-09; dual readout 2026-07-16; stem sidecars 2026-08-20/27.** Lazy adapter for the sibling Kick Detector project. Loads `Kick Detector/Models/kick_crnn_V3.pt` and Kick Detector's reference `model.py` / `presence_postprocess.py` only when `--kick-model` is enabled. One inference returns `KickPresenceReadout(raw, section)`: raw beat presence feeds contextual musical landmarks, while the validated threshold/smoothing (`0.30`, `fill_off_beats=6`, `drop_on_beats=1`) remains the coarse-section signal. The single Demucs pass yields stem envelopes plus raw drums AND bass without double separation. **Stem sidecars:** `__drumsstem.npz` (float32) and `__bassstem.npz` (Opus-bytes-in-npz ~3 MB/track behind `OPUS_PAYLOAD_ENABLED` — Sam's 2026-08-27 arbitration; int16 fallback tested) persist in `_Stem Analysis/`, saved opportunistically whenever a separation runs; source identity is size → mtime → full-content sha1 (`_source_matches`), so byte-identical copies in `Audio Mix N/` hit. The Opus loader re-locates a pilot chirp on every load, reconciles stream length, and verifies up to five timeline-spread source-excerpt anchors — every gate fails closed to a cache miss. Public reader: `load_bass_stem(wav, cache_dir=None)`. Consumers: Kick V3, the beat-grid pass (via `stem_grid._lazy_stem_audio`), and `refit_grid_from_stem.load_or_separate_stems` (cache-first + backfill).
...
**Added 2026-06-08; paired-landmark V2 2026-07-16; tail-loop swap gate 2026-08-14; outgoing-content detector 2026-09-13; rescue-signal defaults flipped 2026-09-22; outgoing-outro-loop targeting fixed 2026-09-22 (D15).** `plan_fill_or_cut`'s outgoing-outro-loop candidate selection now tries named `section:*` boundaries as one full tier BEFORE any raw Kick-Detector-V3 `landmark:*` candidate (was one bar-ascending merged list, so a numerically-smaller landmark could beat a reachable, cleaner section boundary that was never attempted — found by Sam eyeballing a real mix's ALS). The "loop the outro section itself"/"latest clean-drum window" last-resort fallbacks — previously structurally dead code in landmark mode (`nxt`/`chunk` were only ever set together) — are now reachable via `candidate_nxt`/`candidate_target_name` tracking, guarded by `via_d2_fallback` so their lack of the primary search's exact-reach guarantee can't crash the pipeline. `Alignment.outgoing_loop_abandoned` (new field) records when a target was identified but nothing passed the loop-quality gate, distinguishing it from "no loop was needed" in `ARRANGEMENT_REPORT.json`. `report_landmark_candidates` now also emits `section:*` candidates (not just raw landmarks); `propose_arrangement._final_landmark_candidates` tags the real winner `selected: true`. New helpers `_outro_section_target_candidates`/`_outro_landmark_target_candidates`. Full corpus validation + evidence trail: `Documentation/Plans/d15-outro-loop-targeting-plan.md`; replay tool `Tools/d15_outro_loop_replay.py`; tests `Tests/test_outro_loop_targeting.py`. Bass-to-bass alignment engine used by `propose_arrangement`. `paired_landmarks_v2` preserves odd-bar cues, requires paired incoming/outgoing landmarks, suppresses arbitrary incoming-intro loops, and can extend to a named cue up to 64 bars. Cue-bounded tail loops select a clean phrase length that preserves an intermediate swap boundary as well as the final target; candidates whose actual loop interval ends before the locked swap are skipped, with a clear failure if no later named cue fits the repeat/extension caps. Legacy selection retains the 16-48 bar safety gate. `compute_aligned_positions` now also computes `Alignment.outgoing_has_post_swap_content` per pair (via `_outgoing_has_post_swap_content()`, computed AFTER `plan_fill_or_cut` so a loop-extended tail is reflected, not the pre-loop native length) and carries it through the arrangement report for `apply_automation.py`'s margin check — landing-order item (a) of the swap-first redesign (items b/c not built; see `Documentation/BURN_LIST.md` lane C for what's still open). Reads `SECTIONS_STEM_*.json` and retains the transition visualizer. **`CueConfig`'s `tail_anchor_rescue`/`deep_intro_anchor`/`incoming_phrase_anchors` now default `True`** (Sam's long-intro/short-outro rules; burn list D9, 2026-09-22) — Sam's call after `Tools/d9_cue_signal_replay.py` (standalone, sweeps the full 380-pair 14.08.26 corpus comparing two CUE_CONFIG states, mirrors `Tests/test_alignment_baseline.py`'s row shape) proved clean (0 already-OK pairs moved, 87/113 raises rescued). All three flags only ever unlock RESCUE-tier fallback anchor search, tried after the primary drop-anchor search fails — structurally cannot change an already-successful pair.
...
| `Source/bass_residual.py` | **Sizes Sam's "option 2": how much of the OUTGOING's bass to leave in across a swap so the mix does not feel hollow, measured feed-forward with no render.** The quantity is the change the LISTENER hears across the swap - `out_unity+in_kill` before vs `out_kill+in_unity` after - not "outgoing minus incoming", which is a property of two records. Sizes by SEARCH over the shelf ladder (the shelf is a filter straddling sub and bass, so the dB it buys is not a closed form), capped at **Sam's ear-set -9.5 dB** (`RESIDUAL_CEILING` 0.335, a LISTENING ruling refined 2026-09-01 from the -5.68 launch value he called "clashy" - the residual keeps the WEIGHT, never the bass line; pinned by test). A clean run to the cap fires on his verdict even when predicted recovery is small; a guard-stopped run still demands recovery beat the model error. Returns a `ResidualDecision` for EVERY pair including refusals, so a build log can be read. Consumed by `apply_automation` behind `BASS_RESIDUAL_ENABLED`, **default OFF**. Guards: an ABSOLUTE overshoot bound vs the pre-swap level evaluated at 5 probes across the taper; refuses QUICK_SWAP (its Utility Gain is zero at the swap, so a residual would be inaudible); takes the LUFS levelling trims the mix ships with, not the arranged set's 0 dB faders. **Caveat: the firing gate borrows `BAND_P95_DB`, which was measured on SUMMED predictions, while what is gated is a difference of two per-track SHARES - errors that cancel in a sum can add in a difference. Widened by sqrt(2) as a stand-in; clearing it needs held-out solo renders from a second mix.** Added by: Claude. |
