...
def plan_transitions(tracks: list[TrackInfo], report_swaps: dict | None = None) -> list[TransitionPlan]:
    ...
    for i in range(len(tracks) - 1):
        ...
        if rep is not None:
            ...
            outgoing_has_content = rep.get("outgoing_has_post_swap_content", True)
            margin = (4.0 if (not outgoing_has_content or (ov_end - ov_start) / 4 < 24)
                      else 8.0)
            ...
            if aligner_chosen:
                ...
                if abs(clamped - swap) > 1e-6:
                    raise ValueError(
                        ...
                        f"{outgoing_has_content}). This swap does not leave "
                        f"the outgoing's fade the room this margin requires — "
